...
# There are three ansible inventory specific options that can be set in
...
# metadata attribute.
# It is possible to get a /etc/hosts snippet with --hostfile option
...
def append_hostvars(hostvars, groups, key, server, namegroup=False):
    hostvars[key] = dict(
        private_ipv4=server['private_v4'],
        ansible_host=server['public_v4'] or server['private_v4'],
        storage_dev=server['storage_dev'],
        openstack=server)
...
def get_data_storage_dev(inventory, server):
    for cloud in inventory.clouds:
        for volume in cloud.get_volumes(server):
            if 'data_storage' in volume.metadata and volume.metadata.data_storage:
                return cloud.get_volume_attach_device(volume, server['id'])
...
def get_host_groups_from_cloud(inventory):
    ...
    for server in inventory.list_hosts(**list_args):
        ...
        if 'interface_ip' not in server:
            continue
        server['storage_dev'] = get_data_storage_dev(inventory, server)
...
def get_hostfile(inventory):
    out = ['## begin hosts generated by openstack_inventory.py ##']
    out.extend(
        '{}\t{}'.format(host.private_v4.ljust(16), host.name)
        for host in inventory.list_hosts()
    )
    out.append('## end hosts generated by openstack_inventory.py ##')
    return '\n'.join(out)
...
def parse_args():
    ...
    group.add_argument('--hostfile', action='store_true',
