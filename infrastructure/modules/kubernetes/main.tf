...
# K8S Worker load balancer
resource "openstack_lb_loadbalancer_v2" "k8s_load_balancer" {
  count = var.enabled && length(var.load_balancer_ports) > 0 ? 1 : 0
  name = "kubernetes-lb"
  vip_subnet_id = openstack_networking_subnet_v2.k8s_subnet[0].id
}

resource "openstack_lb_listener_v2" "k8s_load_balancer_listener" {
  ...
  name = format("kubernetes-lb-lsnr-%d", var.load_balancer_ports[count.index]["src"])
  loadbalancer_id = openstack_lb_loadbalancer_v2.k8s_load_balancer[0].id
}

resource "openstack_lb_pool_v2" "k8s_load_balancer_pool" {
  count = var.enabled && length(var.load_balancer_ports) > 0 ? length(var.load_balancer_ports) : 0
  name = format("kubernetes-lb-lsnr-pool-%d", var.load_balancer_ports[count.index]["src"])
  protocol    = "TCP"
  lb_method   = "SOURCE_IP"
  listener_id = openstack_lb_listener_v2.k8s_load_balancer_listener[count.index].id
  admin_state_up = true
}

resource "openstack_lb_monitor_v2" "k8s_load_balancer_monitor" {
  count = var.enabled && length(var.load_balancer_ports) > 0 ? length(var.load_balancer_ports) : 0
  name = format("kubernetes-lb-mon-%d", var.load_balancer_ports[count.index]["dst"])
  pool_id = openstack_lb_pool_v2.k8s_load_balancer_pool[count.index].id
  type = "TCP"
  delay = 30
  timeout = 10
  max_retries = 5
...
resource "openstack_lb_member_v2" "k8s_load_balancer_members" {
  count = var.enabled && length(var.load_balancer_ports) > 0 ? length(var.load_balancer_ports) * lookup(var.k8s_worker_instance, "num_instances", 0) : 0
  pool_id = openstack_lb_pool_v2.k8s_load_balancer_pool[ count.index  % length(var.load_balancer_ports) ].id
  address = element(
    openstack_compute_instance_v2.k8s_worker_instance.*.access_ip_v4,
    count.index  % lookup(var.k8s_worker_instance, "num_instances", 0),
  )
  subnet_id = openstack_networking_subnet_v2.k8s_subnet[0].id
