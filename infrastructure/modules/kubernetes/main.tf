...
resource "openstack_networking_secgroup_rule_v2" "k8s_master_secgroup_rule" {
  ...
  direction = "ingress"
  ethertype = "IPv4"
  protocol = "${lookup(var.k8s_master_sec_rules[count.index], "protocol")}"
  port_range_min = "${lookup(var.k8s_master_sec_rules[count.index], "from")}"
  port_range_max = "${lookup(var.k8s_master_sec_rules[count.index], "to")}"
  remote_ip_prefix = "${lookup(var.k8s_master_sec_rules[count.index], "cidr")}"
...
resource "openstack_networking_secgroup_rule_v2" "k8s_worker_secgroup_rule" {
  ...
  direction = "ingress"
  ethertype = "IPv4"
  protocol = "${lookup(var.k8s_worker_sec_rules[count.index], "protocol")}"
  port_range_min = "${lookup(var.k8s_worker_sec_rules[count.index], "from")}"
  port_range_max = "${lookup(var.k8s_worker_sec_rules[count.index], "to")}"
  remote_ip_prefix = "${lookup(var.k8s_worker_sec_rules[count.index], "cidr")}"
  security_group_id = "${openstack_networking_secgroup_v2.k8s_worker_secgroup.id}"
}
...
resource "openstack_networking_port_v2" "k8s_master_port" {
  count = "${var.enabled ? lookup(var.k8s_master_instance, "num_instances", 0) : 0}"
  description = "${format("%s-%02d networking port", local.k8s_master_node_name, count.index + 1)}"
...
resource "openstack_compute_floatingip_associate_v2" "k8s_master_floatingip_association" {
  ...
  instance_id = "${element(openstack_compute_instance_v2.k8s_master_node.*.id, count.index)}"
}

# Kubernetes master node volume
resource "openstack_blockstorage_volume_v2" "k8s_master_volume" {
  count = "${var.enabled ? lookup(var.k8s_master_instance, "num_instances", 0) : 0}"
  name = "${format("%s-%02d", local.k8s_master_volume_name, count.index + 1)}"
  size = "${var.k8s_master_instance["external_volume_size"]}"
}
...
resource "openstack_compute_instance_v2" "k8s_worker_node" {
  ...
  key_pair = "${data.openstack_compute_keypair_v2.ssh_keypair.name}"
  network {
    port = "${element(openstack_networking_port_v2.k8s_worker_port.*.id, count.index)}"
  }
  metadata = {
    ansible_user = "${var.ssh_user}"
...
resource "openstack_networking_port_v2" "k8s_worker_port" {
  count = "${var.enabled ? lookup(var.k8s_worker_instance, "num_instances", 0) : 0}"
  description = "${format("%s-%02d networking port", local.k8s_worker_node_name, count.index + 1)}"
  name = "${format("%s-%02d", local.k8s_worker_port_name, count.index + 1)}"
  network_id = "${openstack_networking_network_v2.k8s_network.id}"
  admin_state_up = true
  security_group_ids = ["${openstack_networking_secgroup_v2.k8s_worker_secgroup.id}"]
  fixed_ip = {
    subnet_id = "${openstack_networking_subnet_v2.k8s_subnet.id}"
...
resource "openstack_compute_floatingip_associate_v2" "k8s_worker_floatingip_association" {
  ...
  instance_id = "${element(openstack_compute_instance_v2.k8s_worker_node.*.id, count.index)}"
}

# Kubernetes worker node volume
resource "openstack_blockstorage_volume_v2" "k8s_worker_volume" {
  count = "${var.enabled ? lookup(var.k8s_worker_instance, "num_instances", 0) : 0}"
...
resource "openstack_compute_volume_attach_v2" "k8s_worker_volume_attach" {
  count = "${var.enabled ? lookup(var.k8s_worker_instance, "num_instances", 0) : 0}"
  volume_id = "${element(openstack_blockstorage_volume_v2.k8s_worker_volume.*.id, count.index)}"
  instance_id = "${element(openstack_compute_instance_v2.k8s_worker_node.*.id, count.index)}"
}
