...
resource "openstack_lb_loadbalancer_v2" "load_balancer" {
  ...
  vip_subnet_id = openstack_networking_subnet_v2.cluster_subnet[0].id
}
...
resource "openstack_lb_member_v2" "load_balancer_members" {
  count = var.enabled && length(var.load_balancer_ports) > 0 ? length(var.load_balancer_ports) * lookup(var.cluster_instance, "num_instances", 0) : 0
  pool_id = openstack_lb_pool_v2.load_balancer_pool[ count.index  % length(var.load_balancer_ports) ].id
  address = element(
    openstack_compute_instance_v2.cluster_instance.*.access_ip_v4,
