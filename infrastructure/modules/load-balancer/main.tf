resource "openstack_lb_loadbalancer_v2" "load_balancer" {
  count              = var.enabled && length(var.lb_members) > 0 ? 1 : 0
  name               = var.lb_name
...
resource "openstack_lb_listener_v2" "load_balancer_listener" {
  count           = var.enabled && length(var.lb_members) > 0 && length(var.lb_ports) > 0 ? length(var.lb_ports) : 0
  protocol        = "TCP"
...
resource "openstack_lb_member_v2" "load_balancer_members" {
  count          = var.enabled && length(var.lb_members) > 0 && length(var.lb_ports) > 0 ? length(var.lb_ports) * length(var.lb_members) : 0
